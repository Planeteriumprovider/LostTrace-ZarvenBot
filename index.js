  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    setTimeout(async () => {
      try {
        ...
        console.log('Gib diesen Code in WhatsApp unter "Verknüpfte Geräte" ein.');
        console.log('======================================\n');
      } catch (error) {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (command === '!codex') {
        ...
        const text = `*LostTrace | Kodex Prüfung*\n\nBeantworte die folgende Aussage mit *!codex answer true* oder *!codex answer false*:\n\n"${questionObj.q}"`;
        await sock.sendMessage(remoteJid, { text }, { quoted: msg });
      } 
      else if (command.startsWith('!codex answer ')) {
        ...
        if (parts.length >= 3) {
          ...
          if (!correctAnswer) {
            await sock.sendMessage(remoteJid, { text: "Es läuft gerade keine Kodex-Prüfung. Starte eine mit *!codex*." }, { quoted: msg });
            ...
            await sock.sendMessage(remoteJid, { text: "Bitte antworte exakt mit *!codex answer true* oder *!codex answer false*." }, { quoted: msg });
          }
        }
      }
        ...
        const textToSend = incomingText.substring(5);
        let mentions = [];
